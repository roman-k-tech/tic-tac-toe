...
settings = Settings()
game_field = settings.create_field()
players = settings.create_players()
game = Game(
    field=game_field,
    players=players,
    settings=settings,
    messages=Messages(deque([Message('Hello!', False)], maxlen=settings.messages['max_count'])),
    status=True,
    active_cell=None,
    screen=ChainMap({}, {}, {}, {}),  # PlayersScreen, FieldScreen, MessagesScreen
)
