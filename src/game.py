...
class Game:
    def __init__(self, field: Field, players: Players, settings: Settings, messages: Messages, status, screen: ChainMap,
                 active_cell):
        self.field = field
    ...
    def get_messages_screen(self):
        ...
        itr_prepared_messages = iter(prepared_messages)
        screen = {}
        for row in range(upper_left_corner.row, lower_right_corner.row):
            for column in range(upper_left_corner.column, lower_right_corner.column):
                try:
                    screen[ScreenCoordinates(row, column)] = ScreenSymbol(
                        next(itr_prepared_messages)
                    )
    ...
    def get_active_cell_screen(self):
        ...
        self.screen.maps[0] = active_cell_screen
    ...
    def is_game_over(self) -> Self:
        ...
        if not self.is_empty_cells(self.field):
            self.messages.append(Message('Всі клітинки зайняті. Нічия!', False))
            self.status = False
        ...
        for row in range(self.settings.field['size_rows']):
            ...
            if winner := self.is_n_symbols_continuously(cells, self.settings.field['win_rows']):
                self.messages.append(Message(f"Переміг {winner}!", False))
                self.status = False
    ...
    def add_player_to_field_position(self, position: FieldCoordinates) -> Self:
        ...
        if self.field[position] is None:
            ...
            self.messages.append(Message(f"Гравець {player} зробив хід", False))
            self.players.rotate()
            self.messages.append(Message(f"Гравець {self.players[0]} наступний", False))
        else:
            self.messages.append(Message("Ця клітинка зайнята. Спробуйте ще раз", False))
